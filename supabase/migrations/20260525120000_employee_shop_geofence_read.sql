-- Allow authenticated employees to read geofence coords for their approved shop only.

drop policy if exists "shops_select_employee_device_shop" on public.shops;
create policy "shops_select_employee_device_shop"
  on public.shops
  for select
  to authenticated
  using (
    exists (
      ...
      where d.shop_id = shops.id
        and d.employee_id = public.employee_id_for_auth_user()
        and d.approved = true
        and coalesce(d.blocked, false) = false
    )
  );

-- Enrich dashboard snapshot with shop geofence for mobile punch UI.
...
begin
  ...
  if v_shop_id is not null then
    select s.name, s.lat, s.lng, s.radius_meters
      into v_shop_name, v_shop_lat, v_shop_lng, v_shop_radius_m
    from public.shops s
  ...
  return jsonb_build_object(
    ...
    'shop_lat', v_shop_lat,
    'shop_lng', v_shop_lng,
    'shop_radius_meters', coalesce(v_shop_radius_m, 150),
